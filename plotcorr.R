...
plot.corr <- function(x,breaks=NULL,reorder=TRUE,colors="Spectral",absc=TRUE,method="ARSA") {
  ...
  if (reorder) {
    ...
    ord <- seriate(distx,method=method)
    ## Thanks to chris.hanretty@gmail.com for the fix
    x <- x[get_order(ord,1),get_order(ord,1)]
  }
